    ...
    class Propellant
      ...
      def ppu
        { Kethane: 0.125, LiquidFuel: 4/9.to_f, XenonGas: 0.5, SolidFuel: 0.05,
          ...
          Water: 0.1, nuclearFuel: 0.1, SpareParts: 40 }[name.to_sym].to_f
      end
      ...
      def cost(mass_ratio)
        temp = ( amount * (5.7 + log_cost * mass_ratio) * ppu / 3 ).round(0)
        if amount != max_amount
          temp += ( max_amount * (5.7 + Math.log(max_amount, 10) * mass_ratio) * 
                   ppu / 12 ).round(0)
        end
        temp
      end
      ...
      def self.allowed_resources
        [ "LiquidFuel", "SolidFuel", "Oxidizer", "XenonGas", "ElectricCharge", "Kethane",
          "MonoPropellant", "Water", "nuclearFuel", "SpareParts" ]
      end
    end
  end
end
